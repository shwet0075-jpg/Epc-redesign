    ...
    <>
      <section className="hero hero--redesigned" aria-label={activeSlide.title}>
        <div className="hero-media" aria-hidden="true">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              initial={shouldReduceMotion ? { opacity: 1 } : { opacity: 0, scale: 1.12 }}
              animate={{ opacity: 1, scale: 1 }}
              exit={shouldReduceMotion ? { opacity: 0 } : { opacity: 0, scale: 1.04 }}
              transition={{ duration: shouldReduceMotion ? 0.2 : 1.15, ease: [0.22, 1, 0.36, 1] }}
        ...
        <div className="container hero-shell">
          ...
          >
            <motion.p className="eyebrow hero-eyebrow" variants={heroCopy} transition={{ duration: 0.55 }}>
              Prudent EPC Pvt. Ltd.
            </motion.p>
            <AnimatePresence mode="wait">
              <motion.h1
        ...
        <div className="container capabilities-layout">
          ...
          <div className="feature-grid" role="list">
            {capabilities.map((capability, index) => (
              <ScrollReveal key={capability.label} variant="fade-up" delay={index * 0.1} className={`feature-card-wrapper feature-card-wrapper--${index + 1}`}>
                <motion.article className="feature-card" whileHover={{ y: -10 }} transition={{ type: 'spring', stiffness: 280, damping: 22 }} role="listitem">
                  <div className="feature-index" aria-hidden="true">0{index + 1}</div>
                  <div className="feature-icon">{capability.icon}</div>
                  <h3>{capability.label}</h3>
                  <p>{capability.desc}</p>
                  <FiArrowRight className="feature-arrow" aria-hidden="true" />
                </motion.article>
              </ScrollReveal>
            ))}
          </div>
